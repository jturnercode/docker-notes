...
x_F = ((MM["Percent_fem_noeduc"] * MM["popfemale_in_thsnd"]) / MM["pop_in_thsnd"]) * (
    -1
)
...
figara.add_trace(
    go.Bar(y=y_age, x=x_M, name="Male", orientation="h", marker_color="#fab17a")
)

# Adding Female data to the figure
figara.add_trace(
    go.Bar(y=y_age, x=x_F, name="Female", orientation="h", marker_color="#f7cd68")
)
...
for element in DF["Year"].unique().tolist():
    ...
    figure2.add_trace(
        go.Bar(x=dff["rang_age"], y=dff["Percent_fem_noeduc"], name=element)
    )
    ...
    figure2.update_layout(
        {
            "xaxis": {"title": {"text": "Classes_age"}, "title_font": dict(size=15)},
            "yaxis": {
                "title": {"text": "%femmes non_eduqués"},
...
figure1.update_layout(
    {
        "xaxis": {"title": {"text": "Années"}, "title_font": dict(size=13)},
        "yaxis": {
            "title": {"text": "%de femmes non eduqués"},
...
figure1.add_trace(
    go.Scatter(
        x=DF["Year"].unique().tolist(), y=K["Percent_fem_noeduc"], name="Tendance"
    )
)
figure1.update_layout(hovermode="x unified")
figure1.update_layout(
    hoverlabel=dict(bgcolor="#f8c86c", font_size=11, font_family="Rockwell")
)

figure3 = px.scatter(
    data_frame=DF, x="Percent_pop_noeduc", y="pop_in_thsnd", color="rang_age"
)
        ...
        dbc.Nav(
            html.Div(
                children=[
                    ...
                    html.H4(
                        html.P("VISUAL INSIGHT", style={"margin": "0px -69px 16px 0px"})
                    ),
                    html.A(
                        "contact",
                        href="mailto:yahyasgh9@gmail.com",
                        style={
                            ...
                            children=[
                                html.Div(
                                    ...
                                    children=[
                                        html.I(
                                            html.B(
                                                html.U(html.P("GRAPH I :")),
                                                style={"color": "#fbaf7b"},
                                            )
                                        ),
                                        html.I(
                                            html.B(html.P("PLOT TYPE:")),
                                        ...
                                        html.Div(
                                            children=[
                                                ...
                                                html.Div(
                                                    children=[
                                                        html.U(
                                                            html.B(html.P("Scatters:"))
                                                        ),
                                                        dcc.Graph(
                                                            id="sec_ex1", figure=figuro
                                                        ),
                                                    ]
                                                ),
                                            ]
...
def interactive_graphs(selection):
    ...
    if selection == "Pie chart":
        fig = px.pie(
            values=K["Percent_fem_noeduc"].tolist(),
            names=DF["Year"].unique().tolist(),
            title="Pie Chart",
            color_discrete_sequence=px.colors.sequential.Redor,
        )
        fig.update_layout(
            {"xaxis": {"title_font": dict(size=13)}},
            plot_bgcolor="white",
            paper_bgcolor="white",
            font_color="black",
            font_family="Times New Roman",
...
@app.callback(
    Output(component_id="sec_ex", component_property="figure"),
...
def interactive_graph(seelection):
    ...
    if seelection:
        ...
        x_FF = (
            (kk["Percent_fem_noeduc"] * kk["popfemale_in_thsnd"]) / kk["pop_in_thsnd"]
        ) * (-1)
        # Creating instance of the figure
        figario = go.Figure()
        # Adding Male data to the figure
        figario.add_trace(
            go.Bar(
                y=y_age, x=x_MM, name="Male", orientation="h", marker_color="#fab17a"
            )
        )
        ...
        figario.add_trace(
            go.Bar(
                y=y_ag, x=x_FF, name="Female", orientation="h", marker_color="#f7cd68"
            )
        )
...
@app.callback(
    Output(component_id="sec_ex1", component_property="figure"),
    Input(component_id="title_d2", component_property="value"),
)
